...
| **herdr fleet layer** | [herdr](https://herdr.dev) is the fleet control plane: `team-up`/`hub-team` spawn peer teams as herdr workspaces via the shared socket client (`.pi/harnesses/lib/herdr-client.ts`) + pure layout module (`scripts/lib/herdr-layout.ts`); coms presence goes push over herdr events; `team-snapshot`/`team-down`/`team-resume` restore whole teams with conversations; the agent-hub dispatcher gains `herdr_*` fleet tools (damage-control-guarded). tmux is fully dropped — herdr is a hard dependency for fleet recipes (non-fleet recipes are unaffected). | `.pi/harnesses/lib/`, `scripts/lib/`, `scripts/team-up.ts`, `scripts/team-snapshot.ts` |
| **Claude Code coms bridge** | Fork-only capability: an interactive Claude Code pane becomes a first-class bidirectional coms peer — `coms-cli` (envelope CLI for non-pi processes), `coms-claude-bridge` daemon, Stop hook, `peer-coms` skill, and `runner: claude-code` peers in peers.yaml. See `docs/claude-code-coms-bridge.md`. | `scripts/coms-cli.ts`, `scripts/coms-claude-bridge.ts`, `hooks/coms-stop-hook.mjs`, `skills/peer-coms/` |
| **Hermes coms bridge + conductor** | Fork-only Hermes integration: `user-remote` bridge sends hub questions to Telegram through Hermes, the ask-user-remote wrapper races local overlay vs. phone answers, and Hermes liaison/conductor skills support answer-file replies plus no-herdr peer-team conduction. See `docs/coms-hermes-bridge.md`. | `scripts/coms-hermes-bridge.ts`, `scripts/lib/hermes-bridge-core.ts`, `.pi/harnesses/ask-user-remote/`, `hermes/skills/`, `docs/coms-hermes-bridge.md` |
...
- **2026-07-08** — **Hermes coms bridge release set shipped** (fork-only capability). `coms-hermes-bridge` registers `user-remote` and relays hub questions to Telegram through `hermes send`; the ask-user-remote wrapper races local pi overlay answers against phone answers and cancels the loser; Hermes `hub-liaison` writes answer files and `hub-conductor` drives live pi peers through `coms-cli` without touching herdr. New reference: `docs/coms-hermes-bridge.md`.
- **2026-07-07** — **Coms-backed dispatch shipped** (`.pi/agents/dispatch-policy.yaml` + backend routing inside the agent-hub `dispatchAgent()`). A team member preferring coms is served by a live same-name pool peer instead of a native subagent spawn — one `dispatch_agent` API, resolved per dispatch (hub + peers boot in parallel), full return-contract/ASK_USER/history pipeline preserved, `fallback: native|none` semantics. Ships with `plan-reviewer`/`code-reviewer`/`documenter` preferring coms so the `runner: claude-code` reviewer peers serve review dispatches with standing context. Plan: `docs/plans/agent-hub/PLAN-coms-backend-dispatch.md`.
- **2026-07-07** — **Claude Code coms bridge + full fleet layer shipped** (fork-only capability). `coms-cli` + `coms-claude-bridge` make an interactive Claude Code pane a bidirectional coms peer (Stop hook primary, `<<COMS_DONE>>` sentinel fallback; `runner: claude-code` in peers.yaml; `peer-coms` skill — 29th skill). Fleet quality-of-life landed with it: `hub-team`, per-peer `env_file:`, pane identity banners, `team-snapshot`/`team-down`/`team-resume` (pi peers resume conversations via herdr-captured session refs), coms presence over herdr push events, dispatcher `herdr_*` fleet tools, and damage-control rules for every destructive herdr verb. New reference: `references/fleet-coordination-patterns.md`.
