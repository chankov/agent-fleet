...
description: Creates specs before coding. Use when starting a new project, feature, or significant change and no specification exists yet. Use when drafting a PRD or requirements document with objectives and scope, or when requirements are unclear, ambiguous, only exist as a vague idea, or need to be grilled before planning. Use when a single requirement spans several independently testable capabilities and needs decomposing into a capability map of modules before specifying.
---
...
**Project overrides:** if `.ai/agent-fleet-overrides.md` has a `## spec-driven-development` section, its keys (`spec-dir`, `naming`) override these defaults. Absent file or section → use the defaults above. See [docs/agent-fleet-setup.md](../../docs/agent-fleet-setup.md).
...
**Then recurse per module.** Run Specify → Plan → Tasks → Implement for each module in dependency order. Each module gets its own spec, scoped to that module's objective, boundaries, and success criteria. Save the approved map in the same spec directory the Output Convention above resolves to (default `docs/prds/{area}/`), as `CAPABILITY-MAP-{topic}.md`, and give each module its own PRD under the project's normal naming with the module id as the `{topic}` slug. The map, not filename guessing, is the index of what exists.
