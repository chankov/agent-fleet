...
ROOT = pathlib.Path(__file__).resolve().parents[6]  # repo root, above .pi/agent-fleet
...
class ReviewLiveWiringRedTest(unittest.TestCase):
    ...
    def test_plugin_uses_fastapi_query_parameter_names_end_to_end(self):
        source=(ROOT/".pi/agent-fleet/hermes/desktop-plugins/agent-fleet-monitor/plugin.js").read_text()
    ...
    def test_cancel_response_has_an_explicit_bounded_wire_contract(self):
        # The server moved to .pi/harnesses/lib/; .pi/agent-fleet/scripts/lib/ kept a two-line
