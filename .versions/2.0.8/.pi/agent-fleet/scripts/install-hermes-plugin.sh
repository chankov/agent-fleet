...
#
#   .pi/agent-fleet/scripts/install-hermes-plugin.sh agent-fleet-herdr [--profile default]
#                                    [--copy] [--uninstall] [--force] [--dry-run]
#
# A plugin has up to two halves, installed into the profile the same way:
#
#   .pi/agent-fleet/hermes/plugins/<id>/          -> <profile>/plugins/<id>          (backend, FastAPI)
#   .pi/agent-fleet/hermes/desktop-plugins/<id>/  -> <profile>/desktop-plugins/<id>  (Desktop pane, ESM)
#
