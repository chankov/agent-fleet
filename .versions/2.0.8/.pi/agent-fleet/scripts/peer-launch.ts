// .pi/agent-fleet/scripts/peer-launch.ts
//
...
import { PERSONA_DIRS } from "./lib/persona-dirs.ts";
...
function personaExists(persona: string): boolean {
	return PERSONA_DIRS.some((dir) => fs.existsSync(path.join(REPO_ROOT, ...dir, `${persona}.md`)));
}
	...
	if (found) {
		console.log(
			`Peer "${plan.name}" is ready in pane ${paneId} after ${Math.round(waitedMs / 1000)}s — ` +
				`address it with: node --experimental-strip-types .pi/agent-fleet/scripts/coms-cli.ts send ${plan.name} "…" --project ${plan.project} --name <you>\n` +
