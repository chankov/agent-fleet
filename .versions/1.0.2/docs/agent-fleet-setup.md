...
| `mode` | `standard` | Execution mode for the hub's per-turn budgets: `fast` (single-specialist path, no nested delegation, assertion ledger optional), `standard` (batched execution with moderate budgets), or `strict` (full Verification Contract with wide budgets). Exhaustion makes `dispatch_agent`/`spawn_research` refuse and request one Yes/No `ask_user` confirmation; Yes renews the budget in the same tool loop, without a typed `continue` or slash command. A normal new user message also opens a fresh turn window. Switchable live with `/af-hub-mode`. |
