...
- If implementation requires a choice not already settled in the task, plan, PRD, chat, prompt, or rules (multiple valid approaches, contradiction, competing code patterns), do not pick silently. Follow `skills/_internal/grilling.md` if it exists and pause with `ASK_USER: <question>` (one focused question, 2–4 options plus a recommended choice). Already-stated requirements are not re-asked. Do not continue the slice in the same turn as an `ASK_USER` request.
