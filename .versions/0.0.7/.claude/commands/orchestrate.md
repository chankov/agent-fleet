...
You are the **orchestrator**. The primary agent drives a team of installed Claude Code
subagents to deliver the task in `$ARGUMENTS`. Subagents cannot nest — **all dispatching is
yours**.
...
1. Read `.claude/orchestrate-teams.yaml` (a map of team-name → ordered persona list).
2. Resolve team + task from `$ARGUMENTS`:
   - Treat the **first token** as a team name **only** when it is a bare single word that
     ...
     team explicitly, the user writes `/orchestrate team=<name> <task>`. Otherwise the
...
3. **Validate personas:** each persona in the resolved team must exist in `.claude/agents/`.
...
run `/review`). When the active team *does* include a reviewer, run it as a normal step and
