...
The interview needs a live user. **Don't run it in non-interactive contexts** — CI, a headless `just flow` run, or any autonomous loop. If constraints are missing and you're in one of those, apply the Floor below, note that you did, and flag the rest for a human.
...
| Existing Fleet gate | `quality:` under `## workflows` in `.ai/agent-fleet-overrides.md` |
| Agent harness | `.pi/`, `.ai/agent-fleet-overrides.md`, `AGENTS.md` |
...
> **In a Fleet project the two halves live apart, on purpose.** `CONSTRAINTS.md` carries
> the *dimensions, the numbers, and the reason for each number* — it is what a human reads
> and what review diffs. The *executable gate* is `quality:` under `## workflows` in
> `.ai/agent-fleet-overrides.md`, which is the one command `just flow` runs; there is no
> implicit fallback, and a missing or empty value is a startup refusal (exit `3`). Write
> the command in `quality:` and cite it from the `Checked by` column. Never let
> `CONSTRAINTS.md` name a gate command that `quality:` does not run — that is the drift
...
Then make the file discoverable. In a Fleet project the agent-facing pointer belongs in
`.ai/agent-fleet-overrides.md` — the harnesses already read it and the deterministic CLI
owns it. Adding the same line to a human-owned `AGENTS.md` or `CLAUDE.md` is optional and
needs the user's say-so first; never edit those two silently.
...
**In a Fleet project, `quality:` points at the task-end script.** Set
`quality: npm run check:task` (or whatever the project's equivalent is) under
`## workflows` in `.ai/agent-fleet-overrides.md`. That is what `just flow` executes, so it
is the one command that actually blocks a flow from being accepted.

The commands now live in two places — the `Checked by` column in `CONSTRAINTS.md` and these scripts. `CONSTRAINTS.md` is the canonical source *for the reasoning*: it carries why each number is what it is, and it shows up in review. The scripts are the mechanism, and `quality:` is the one the runtime obeys. If the file and the scripts drift, the file wins and the scripts get fixed.
...
|-------|---------|-----------|--------|
| BUILD | `/af-build` | Types, lint, secrets, the floor | under 5s, changed file only |
| VERIFY | `/af-test` | Related tests, coverage on changed lines | under 90s |
| REVIEW | `/af-review` | Everything, plus the guards below | minutes |
| SHIP | `/af-ship` | Direction checks, no regressions | CI |
| FLOW | `just flow` | Whatever `quality:` names — the gate that decides acceptance | per flow |
...
Constraints are the *numbers* layered on top of the project-wide Definition of Done — the standing, unnumbered bar every change clears regardless of task. This skill does not restate it; see [`../../references/definition-of-done.md`](../../references/definition-of-done.md). The floor below duplicates none of it: the Definition of Done says the work is finished, the floor says the bar was not quietly lowered to get there.
