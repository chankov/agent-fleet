...
Runtime dependencies (`@modelcontextprotocol/sdk`, `typebox`) are declared at `.pi/extensions/package.json` for cloned/copy installs and duplicated in the root package's production `dependencies` for published-package symlink installs. The duplication is intentional: Node resolves a symlinked extension from its real path inside `@chankov/agent-fleet`, while npm does not install dependencies from a nested `.pi/extensions/package.json`. Run `npm ci` in `.pi/extensions/` for a clone or copied install; a normal `@chankov/agent-fleet` package install supplies the same dependencies at package root. No per-wrapper install is required. `@mariozechner/pi-coding-agent` is supplied by the pi runtime that loads the extension.
