...
- For a successful writable native dispatch, the Hub may attach `details.compilerDiagnostics` plus a full evidence artifact before presenting the return. When a completed, non-overlapped compiler check finds an error in an observed changed file, the runtime-corrected structured return takes precedence over the raw envelope: only `assertions_proven` IDs from that dispatch are moved to `assertions_unproven`, their original evidence is retained, and the reason is `compiler_diagnostics`. Errors only in unchanged consumers, overlap, or incomplete checks remain visible warnings but do not automatically demote an assertion. Compiler success never promotes an assertion, and this correction never mutates the global ledger.
