// .pi/agent-fleet/scripts/lib/peer-launch.ts
//
...
export interface PeerLaunchOptions {
	 ...
	 * --all-extensions under a name that happens to match a persona file.
	 */
	noPersona?: boolean;
	model?: string;
	project?: string;
...
export function buildPeerLaunchPlan(opts: PeerLaunchOptions, ctx: PeerLaunchContext): PeerLaunchPlan {
	...
	if (!persona) {
		if (opts.extensions) {
			throw new Error(
				`--extensions needs a persona peer. Pass --persona <name>, add .pi/agents/personas/${name}.md, ` +
