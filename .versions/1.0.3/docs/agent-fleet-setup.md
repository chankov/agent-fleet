...
| `max-research-per-turn` | tier default (1/2/4/6) | Ceiling on `spawn_research` calls per user turn (the auto-research pipe and the `/af-research` command are exempt). Positive integer or `off` (stays at the tier). |
