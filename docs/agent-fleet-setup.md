...
| `mode` | `standard` | Execution mode for the hub's per-turn budgets: `fast` (single-specialist path, no nested delegation, assertion ledger optional), `standard` (batched execution with moderate budgets), or `strict` (full Verification Contract with wide budgets). Exhaustion makes `dispatch_agent`/`spawn_research` refuse and request one Yes/No `ask_user` confirmation; Yes renews the budget in the same tool loop, without a typed `continue` or slash command. A normal new user message also opens a fresh turn window. Switchable live with `/af-hub-mode`, or together with posture via `/af-work-mode` / **Alt+M**. |
...
| `turn-wall-time-s` | mode default (900/3600/14400) | Active-time budget, in seconds, per user turn; time blocked in `ask_user` is excluded. Once exceeded, further dispatch/research calls refuse until the one-click continuation is accepted or a new user turn begins. Positive integer or `off`. |
