...
| **Tooling reach** | Claude Code, OpenCode, and pi — with pi-native multi-agent harnesses (`agent-hub`, `coms`, `damage-control-continue`) | Multi-tool: Claude Code, Codex, Gemini CLI, OpenCode, Cursor, Copilot CLI, Factory Droid | Claude Code-first (also usable with Codex) |
