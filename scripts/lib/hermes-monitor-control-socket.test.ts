...
test("UDS cancel route authenticates profile token and forwards only task plus generation to the hub-owned control registry", async (t) => {
	const root = mkdtempSync(join(socketTempRoot(), "monitor-control-socket-")); const profile = join(root, "profile"); mkdirSync(profile);
	const calls: unknown[] = [];
...
test("UDS output cap accommodates worst-case escaped retained output", async (t) => {const root=mkdtempSync(join(socketTempRoot(),"monitor-output-cap-")),profile=join(root,"profile");mkdirSync(profile);const registration:any=new MonitorRegistry({runtimeDir:join(root,"runtime")}).register({profilePath:profile,hubInstanceId:"hub",snapshot:()=>({})});registration.output=()=>({text:"\u0000".repeat(256*1024),sequence:1,firstSequence:1,truncated:false});const socket=new MonitorSocketServer(registration);await socket.listen();t.after(()=>socket.close());const body=await controlRequest(registration.socketPath,JSON.stringify({type:"output",token:registration.token,taskId:"task",generation:1,afterSequence:0})+"\n", 5_000);assert.equal(JSON.parse(body).ok,true);});
