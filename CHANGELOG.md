...
- 28025a2: Add opt-in, off-by-default proactive turn review for Hub and native specialists, with bounded snapshots, rule selection, local validators and advisory feedback on natural turns. Package the observer/runtime and add Fleet history, retained evidence and explicit local human-label reports.

  Keep review-history and evidence overlays at the Fleet frame height so dispatcher chat does not show through; wrap missing-evidence text on narrow terminals.

  Setup does not create or activate repository review configuration and preserves existing human-owned settings. Remote review requires separate repository consent. Human-labelled semantic calibration remains outstanding; findings are not validated bugs, and retained findings currently expose label keys rather than a complete checked-clear/miss/true-negative authoring workflow.
...
- c45d22e: Harden Hub recovery, add fail-closed native write isolation, and confine scout to an isolated snapshot.
...
- caddfb2: Add a bounded below-editor fleet widget with safe editor composition, retained completion rows, shared confirmed actions, and lifecycle-aware refresh.
- caddfb2: Unify fleet observability data so peer activity, run timing, and nested delegate hierarchy stay consistent across dashboard and widget views.
- caddfb2: Register Alt+I through Pi's production shortcut path for gated fleet-widget toggling, retain j/k controls, and leave plain and Alt arrows unchanged for the editor.
- cc8fc8b: Rewrite the npm `description` and `keywords` for pi.dev package-catalog discoverability.
