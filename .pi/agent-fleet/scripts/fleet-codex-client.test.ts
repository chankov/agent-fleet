...
test("CLI help is side effect free and invalid operations fail as JSON", t => {
 const f = fixture(t); const cli = path.resolve(".pi/agent-fleet/scripts/fleet-codex-client.ts");
...
test("CLI selects from real registry fixtures and restores on a new invocation", t => {
 ...
 const cli = path.resolve(".pi/agent-fleet/scripts/fleet-codex-client.ts");
...
test('CLI executes when invoked through a symlink or macOS /tmp alias',t=>{
 const f=fixture(t);const link=path.join(f.dir,'client.ts');fs.symlinkSync(path.resolve('.pi/agent-fleet/scripts/fleet-codex-client.ts'),link);
