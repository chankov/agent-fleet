...
test('manifest-loaded hidden dashboard bridge registers a non-placeholder pane implementation', () => {
  const manifest = JSON.parse(fs.readFileSync(path.join(root, '.pi/agent-fleet/hermes/plugins/agent-fleet-monitor/dashboard/manifest.json'), 'utf8'));
  assert.equal(manifest.tab.hidden, true);
  const bundle = fs.readFileSync(path.join(root, '.pi/agent-fleet/hermes/plugins/agent-fleet-monitor/dashboard', manifest.entry), 'utf8');
