...
test("provider/API-specific output caps match installed xAI Responses and OMLX completions semantics", () => {
	const responses = applyProviderOutputTokenCap({ model: "grok-4.6", input: [{ role: "user", content: "small" }], stream: true, store: false }, XAI_MODEL, 2048);
...
test("missing or oversized configured limits and semantic body mismatches fail closed", () => {
	const responses = { model: "grok-4.6", input: [{ role: "user", content: "small" }], stream: true };
	...
	assert.throws(() => applyProviderOutputTokenCap({ model: "grok-4.6", messages: [{ role: "user", content: "small" }], stream: true, max_output_tokens: 1 }, XAI_MODEL, 2048), /payload shape/);
