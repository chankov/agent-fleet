...
1. Reads `version:` from `.ai/agent-fleet-setup.md` in the current working directory. If the record is missing (the deterministic lifecycle has not yet run), the check is skipped.
...
Included by the deterministic Default/Full lifecycle selections when requested by their features, and available directly:
...
- If the install record is *pre-versioning* (no `version:` line), the check is skipped — there's no recorded baseline to compare against. Run `npx @chankov/agent-fleet setup` once to stamp the current version.
