...
1. Reads `version:` from `.ai/agent-fleet-setup.md` in the current working directory. If the record is missing (workspace was never set up via `/af-setup-agent-fleet`), the check is skipped.
...
```
agent-fleet update available: 0.1.0 → 0.2.0. Run "npx @chankov/agent-fleet@latest update" then /af-setup-agent-fleet to apply.
```
...
- If the install record is *pre-versioning* (no `version:` line), the check is skipped — there's no recorded baseline to compare against. Run `/af-setup-agent-fleet` once to stamp the current version.
